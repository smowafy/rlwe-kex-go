...
func EnsureMod(a uint32) uint32 {
	...
	b := uint32(1 ^ utils.BitwiseEq(int64(a), int64(mask)))
...
func ModAdd(a, b uint32) uint32 {
	...
	c += uint32(utils.BitwiseLt(int64(c), int64(a)))
...
func EnsureLongMod(a int64) int64 {
	...
	compRes := utils.BitwiseLt(firstOp, x1)
...
func AdjustPositiveNegative(n int64) int64 {
	...
	return n - (LongMod * (1 - utils.BitwiseLt(n, (LongMod>>1))))
}
...
func ModularRound(n int64) uint32 {
	...
	res := (1 ^ (utils.BitwiseGtOrEqual(nadj, ModularRoundLowerBound) & utils.BitwiseLt(nadj, ModularRoundUpperBound)))

	return uint32(res & 1)
}

func CrossRound(n int64) uint32 {
	a := utils.BitwiseLt(n, CrossRoundQ4)
	b := utils.BitwiseLt(n, CrossRoundQ2)
	c := utils.BitwiseLt(n, CrossRoundQ4+CrossRoundQ2)
...
func Reconciliate(w int64, b uint32) uint32 {
	...
	inRange := utils.BitwiseGtOrEqual(AdjustPositiveNegative(w), AdjustPositiveNegative(IBLowerBound)) &
		utils.BitwiseLt(AdjustPositiveNegative(w), AdjustPositiveNegative(IBUpperBound))
